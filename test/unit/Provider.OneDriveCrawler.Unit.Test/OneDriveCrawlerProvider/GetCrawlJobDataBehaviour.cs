...
{
    public class GetCrawlJobDataBehaviour : OneDriveCrawlerProviderTest
    {
        ...
        [Theory]
        [InlineAutoData]
        public async Task OneDriveCrawlerCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.OneDriveCrawler.Core.OneDriveCrawlerCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
