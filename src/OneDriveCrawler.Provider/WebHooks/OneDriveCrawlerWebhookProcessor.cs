...
{
    public class OneDriveCrawlerWebhookProcessor : BaseWebhookProcessor
    {
        public OneDriveCrawlerWebhookProcessor(ApplicationContext appContext)
            : base(appContext)
        {
        }
        ...
        {
            return webhookDefinition.ProviderId == OneDriveCrawlerConstants.ProviderId || base.Accept(webhookDefinition);
        }
        ...
        public override IEnumerable<Clue> DoProcess(ExecutionContext context, WebhookDataCommand command)
        {
            try
            {
                ...
                if (command.WebhookDefinition.ProviderDefinitionId != null)
                {
                    ...
                    {
                        var crawlJobData = new OneDriveCrawlerCrawlJobData();
                        ...
                        IAgentJobProcessorArguments jobArgs = new DebugAgentJobProcessorArguments
                        {
                            ...
                            Job           = new AgentJob(Guid.NewGuid(), AgentJobPriority.Normal, "CluedIn" + OneDriveCrawlerConstants.ProviderName, ProcessingRestriction.Any, null, null)
                        };

                        var processorState = new AgentJobProcessorState<OneDriveCrawlerCrawlJobData>(jobArgs, AppContext)
                        {
